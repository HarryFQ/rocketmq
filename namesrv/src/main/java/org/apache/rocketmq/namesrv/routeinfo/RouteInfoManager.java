...
public class RouteInfoManager {
    ...
    /**
     * 注册Broker
     * 1. 由于一个NameServer可能同时收到多个Broker的注册请求，所以在处理注册请求时使用了读写锁，在进行修改的时候添加写锁，处理逻辑如下：
    ...
    /**
     * brokerLiveTable保存了当前NameServer收到的心跳数据，遍历brokerLiveTable，获取每一个Broker最近一次发送心跳的时间
     * ，如果上一次发送心跳的时间 + 过期时间（120s） 小于 当前时间，也就是超过120s没有收到某个Broker的心跳包，则认为此Broker已下线，需要关闭该Broker的Channel，将Broker移除.
     *
     *
     */
